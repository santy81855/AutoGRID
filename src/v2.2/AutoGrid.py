...
def runAutoGridZoom():
    ...
    for i in range(3, totals_row):
        ...
        ws['{}{}'.format(colLetter, i)] = '=SUM({}{}:{}{})'.format(numToLetter[3], i, numToLetter[updatedColCount - 2], i)
...
def runAutoGridZoomHelper(wb, ws, attendance_report, file_day):
    ...
    with open(attendance_report) as fin:
        ...
        for index,row in enumerate(reader):
            ...
            last_name = str(row[1])
            cur_day = int(cur_day)
            addToGrid(wb, ws, cur_day, first_name, last_name, isDouble, isReview, True)        
    ...
    if inPerson == True:
        ...
        for i in range(2, ws2.max_row + 1):
            ...
            if curMonth == current_month:
                ...
                addToGrid(wb, ws, curDay, first, last, isDouble, isReview, False)
    '''
    # every time we add a new column, we make sure all the rows have the correct sum formulas
    updatedColCount = ws.max_column
...
def underlineCell(wb, ws, col, row):
    ws['{}{}'.format(numToLetter[col], row)].font = Font(size = 11, underline='single')

def addToGrid(wb, ws, curDay, first_name, last_name, isDouble, isReview, isZoom):
    ...
    if check == 0 and first_blank_row < totals_row:
        ...
        if isZoom == True:
            underlineCell(wb, ws, curCol, first_blank_row)
